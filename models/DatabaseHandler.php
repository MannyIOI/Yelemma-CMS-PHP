...
class DatabaseHandler{
  ...
  public function __construct(){
        ...
        try {
          $this->dbh = new PDO($dsn, $this->user, $this->pass, $options);
            ...
            $this->dbh->query("CREATE DATABASE IF NOT EXISTS yelema")
        or die(print_r($this->$dbh->errorInfo(), true));
        $this->dbh->query("use yelema");
  ...
  `approved` tinyint(4) NOT NULL
    ...
    public function validate($userid,$password){
      $statement = "SELECT * FROM user_data WHERE user_id = '" .$userid. "' AND password = '" .$password. "' LIMIT 1";
